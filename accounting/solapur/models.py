...
class Patient(models.Model):
    ...
    center=models.CharField(max_length=7, default='Solapur')
    mr_uid=models.IntegerField(default=None)

    @property
    def patient_number(self):
        if self.pk:
            return "{}{:d}".format('PN/S/', self.pk)
        else:
            return ""
...
class OPDCashSheet(models.Model):
    ...
    balance=models.DecimalField(max_digits=7, decimal_places=2, default=0, null=True, blank=True)
    ...
    def prev_bal(self):
        if not self.pk:
            try:
                print('aaya ki nahin')
                return OPDCashSheet.objects.order_by('-id').first()
            except OPDCashSheet.DoesNotExist:
                print('ya phir yeh aaya')
                return None
        ...
        if not self.pk:
            if self.cash_in:
                ...
                if self.mode=='cash':
                    ...
                    if self.prev_bal:
                        print('Hello 04')
                        self.balance=self.prev_bal.balance+self.cash_in
                        print('Hello 05')
                    else:
                        print('Hello 06')
                        self.balance=self.cash_in
                        print('Hello 07')
            elif self.cash_out:
                print('Hello 08')
                if self.patient==None:
                    ...
                    if self.prev_bal:
                        print('Hello 09')
                        self.balance=self.prev_bal.balance-self.cash_out
                        print('Hello 10')
                    else:
                        print('Hello 11')
                ...
                if self.prev_bal:
                    print('Hello 101')
                    self.balance=self.prev_bal.balance
                    print('Hello 102')
                else:
                    pass

    def cash_out_bal(self):
        if self.pk:
            cls = self.__class__
            print('cls ka value: ', cls)
            print('cls._meta.get_fields() ka value: ', cls._meta.get_fields())
            old = cls.objects.get(pk=self.pk)
            print('old ka value: ', old.cash_out)
            new = self
            print('new ka value: ', new.cash_out)
            try:
                print('Try here')
                if old.cash_out != new.cash_out:
                    print('mic check 100')
                    oldie=old.cash_out
                    print('mic check 101')
                    newbie=new.cash_out
                    print('mic check 102')
                    new_bal=oldie-newbie
                    print('mic check 103')
                    print(new_bal)
...
class BankDeposits(models.Model):
    ...
    mode=models.CharField(max_length=5, default=None)
    bank_ac=models.CharField(max_length=25, default='OOPL Pvt. 50200021888130')
    branch=models.CharField(max_length=20, default='HDFC Solapur Branch')
    remarks=models.TextField(max_length=500, blank=True, null=True, default=None)
    created_on=models.DateTimeField(auto_now_add=True)
    modified_on=models.DateTimeField(auto_now=True)
...
class ApprovedPackage(models.Model):
    ...
    treatment=models.ForeignKey(Treatment, on_delete=CASCADE, default=None)
...
class CTScan(models.Model):
    ...
    patient_type=models.ForeignKey(PatientType, on_delete=CASCADE, default=None)
    treatment=models.ForeignKey(Treatment, on_delete=CASCADE, default=None)
    pattern = RegexValidator(r'(RT|rt|rT|Rt)[0-9]{2}\/[0-9]{6}', 'Enter RT Number properly!')
...
class Radiations(models.Model):
    ...
    def package(self):
        try:
            return Package.objects.filter(patient=self.patient, patient_type=self.patient_type, treatment=self.treatment).order_by('-id').first()
    ...
    def approved(self):
        try:
            return ApprovedPackage.objects.filter(patient=self.patient, patient_type=self.patient_type, treatment=self.treatment, approval_date__gte=self.package.date_of_admission).order_by('-id').first()
...
class Realization(models.Model):
    ...
    def radiations(self):
        try:
            return Radiations.objects.filter(patient=self.patient, patient_type=self.patient_type, treatment=self.treatment).order_by('-id').first()
    ...
    def radiations_old(self):
        try:
            return Radiations.objects.filter(patient=self.patient, patient_type=self.patient_type, treatment=self.treatment, radiations_date__lte=self.realization_date).order_by('-id').first()
        except Radiations.DoesNotExist:
            return None
    ...
    def discharge(self):
        try:
            return Discharge.objects.filter(patient=self.patient, patient_type=self.patient_type, treatment=self.treatment).order_by('-id').first()
...
class LockData(models.Model):
    ...
    treatment=models.ForeignKey(Treatment, on_delete=CASCADE, default=None, blank=True, null=True)
...
class IpdReport(models.Model):
    ...
    def track_id(self):
        if self.pk:
            return "{}{:d}".format('OOPL/S/', self.pk)
        else:
            return ""
    @property
    def previous(self):
