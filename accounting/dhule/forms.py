...
class CommonExpensesForm(ModelForm):
    ...
    paid_by=forms.CharField(label='Paid By', widget=forms.RadioSelect(choices=PARTY_SELECT, attrs={'class': 'form-check-inline'}))
    class Meta:
...
class ApprovedPackageForm(ModelForm):
    ...
    def clean(self):
        ...
        if not self.instance.pk:
            ...
            if Discharge.objects.filter(patient=pt, patient_type__patient_type=ptt):
                dis1=Discharge.objects.filter(patient=pt, patient_type__patient_type=ptt)
                print('approved package form mein dis1 ka value: ', dis1)
                try:
                    pkg1=Package.objects.filter(patient=pt, patient_type__patient_type=ptt)
                ...
                try:
                    Package.objects.get(patient=pt, patient_type__patient_type=ptt)
                    print(Package.objects.get(patient=pt, patient_type__patient_type=ptt))
                except Package.DoesNotExist:
            ...
            if LockData.objects.filter(patient=self.instance.patient, patient_type__patient_type=self.instance.patient_type.patient_type, date__gte=self.instance.approval_date).order_by('id').first():
...
class CTScanForm(ModelForm):
    ...
    def clean(self):
        ...
        if not self.instance.pk:
            ...
            if Discharge.objects.filter(patient=pt, patient_type__patient_type=ptt):
                ...
                try:
                    ...
                    if not pkg2:
                        ...
                        try:
                            ap1=ApprovedPackage.objects.filter(patient=pt, patient_type__patient_type=ptt)
                ...
                try:
                    ...
                    print(Package.objects.get(patient=pt, patient_type__patient_type=ptt))
                    try:
                        ApprovedPackage.objects.get(patient=pt, patient_type__patient_type=ptt)
                        print(ApprovedPackage.objects.get(patient=pt, patient_type__patient_type=ptt))
                    except ApprovedPackage.DoesNotExist:
            ...
            if LockData.objects.filter(patient=self.instance.patient, patient_type__patient_type=self.instance.patient_type.patient_type, date__gte=self.instance.date).order_by('id').first():
                lock=LockData.objects.filter(patient=self.instance.patient, patient_type__patient_type=self.instance.patient_type.patient_type, date__gte=self.instance.date).order_by('id').first()
            ...
            if Discharge.objects.filter(patient=pt, patient_type__patient_type=ptt):
                    ...
                    if not pkg2:
                        ...
                        try:
                            ...
                            if not ap2:
                                ...
                                try:
                                    ct1=CTScan.objects.filter(patient=pt, patient_type__patient_type=ptt, date__gte=ap2.approval_date)
                                    if not ct1:
                ...
                try:
                    ...
                    try:
                        ...
                        print(ApprovedPackage.objects.get(patient=pt, patient_type__patient_type=ptt))
                        try:
                            CTScan.objects.get(patient=pt, patient_type__patient_type=ptt)
                            print(CTScan.objects.get(patient=pt, patient_type__patient_type=ptt))
                        except CTScan.DoesNotExist:
...
class DischargeForm(ModelForm):
    discharge_updated=forms.BooleanField(label='Discharge Updated?', widget=forms.CheckboxInput(attrs={'class':'form-check-input'}))
    class Meta:
        model=Discharge
        fields='__all__'
        labels={
    ...
    def clean(self):
        ...
        if not self.instance.pk:
            ...
            radiations=Radiations.objects.filter(patient=pt, patient_type__patient_type=ptt).order_by('-id').first()
            if Discharge.objects.filter(patient=pt, patient_type__patient_type=ptt, date_of_discharge__gt=radiations.date).exists():
            ...
            if LockData.objects.filter(patient=self.instance.patient, patient_type__patient_type=self.instance.patient_type.patient_type, date__gte=self.instance.date_of_discharge).order_by('id').first():
                lock=LockData.objects.filter(patient=self.instance.patient, patient_type__patient_type=self.instance.patient_type.patient_type, date__gte=self.instance.date_of_discharge).order_by('id').first()
...
class RealizationForm(ModelForm):
    ...
    def clean(self):
        ...
        if not self.instance.pk:
            ...
            if Discharge.objects.filter(patient=pt, patient_type__patient_type=ptt):
                print('clean1')
                dis=Discharge.objects.filter(patient=pt, patient_type__patient_type=ptt).order_by('-id').first()
                pkg=Package.objects.filter(patient=pt, patient_type__patient_type=ptt).order_by('-id').first()
                rec=ApprovedPackage.objects.filter(patient=pt, patient_type__patient_type=ptt).order_by('-id').first()
                ...
                try:
                    pkgpt=Package.objects.get(patient=pt, patient_type__patient_type=ptt)
                    ...
                    raise ValidationError('There is no Package value for this entry - 2!')
            try:
                ap1=ApprovedPackage.objects.filter(patient=pt, patient_type__patient_type=ptt).order_by('-id')[1]
                try:
            ...
            if LockData.objects.filter(patient=self.instance.patient, patient_type__patient_type=self.instance.patient_type.patient_type, date__gte=self.instance.billing_month).order_by('id').first():
                lock=LockData.objects.filter(patient=self.instance.patient, patient_type__patient_type=self.instance.patient_type.patient_type, date__gte=self.instance.billing_month).order_by('id').first()
...
class LockDataForm(ModelForm):
    lock=forms.BooleanField(widget=forms.CheckboxInput(attrs={'class':'form-check-input'}))
    class Meta:
        model=LockData
        fields='__all__'
        labels={
...
class DepositorForm(ModelForm):
    class Meta:
        model=Depositor
        fields='__all__'
        widgets={

           'depositor': forms.TextInput(attrs={'class':'form-control', 'autofocus':True}),
...
class PiggyBankForm(ModelForm):
    class Meta:
        ...
        widgets={
            ...
            'depositor': forms.Select(attrs={'class':'form-control'}),
            'date': forms.DateInput(attrs={'type': 'date', 'class':'form-control'}),
            'amount': forms.NumberInput(attrs={'class':'form-control'}),
...
class BillGateway(forms.Form):
    ...
    to_date=forms.DateField(widget=forms.DateInput(attrs={'type': 'date', 'class':'form-control', 'id': 'to'}), label='To ')
    billing_date=forms.DateField(widget=forms.DateInput(attrs={'type': 'date', 'class':'form-control', 'id': 'to'}), label='Date of Billing ')

class AdditionalFields(forms.Form):
    change_rt_cash_desc=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit RT Cash (KCC) Description', required=False)
    change_rt_cash_amount=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit RT Cash (KCC) Amount", required=False)
    ...
    change_partial_cash_desc=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Partial Cash (KCC) Description', required=False)
    change_partial_cash_amount=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Partial Cash (KCC) Amount", required=False)
    ...
    change_mjpjay_desc=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit MJPJAY (KCC) Description', required=False)
    change_mjpjay_amount=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit MJPJAY (KCC) Amount", required=False)
    ...
    change_pmjay_desc=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit PMJAY (KCC) Description', required=False)
    change_pmjay_amount=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit PMJAY (KCC) Amount", required=False)
    ...
    change_police_file_desc=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Police File (KCC) Description', required=False)
    change_police_file_amount=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Police File (KCC) Amount", required=False)
    ...
    change_insurance_desc=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Insurance (KCC) Description', required=False)
    change_insurance_amount=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Insurance (KCC) Amount", required=False)
    ...
    change_other_income_desc=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Other Income (KCC) Description', required=False)
    change_other_income_amount=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Other Income (KCC) Amount", required=False)
    ...
    change_ct_kcc_desc=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit CT (KCC) Description', required=False)
    change_ct_kcc_amount=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit CT (KCC) Amount", required=False)
    ...
    change_mould_kcc_desc=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Mould (KCC) Description', required=False)
    change_mould_kcc_amount=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Mould (KCC) Amount", required=False)
    change_additional_income_desc=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Additional Income (KCC) Description', required=False)
    kcc_share_add_in1=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)
    ...
    kcc_share_add_in2=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)
    change_other_expenses_desc=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Other Expenses (KCC) Description', required=False)
    kcc_share_other_expenses1=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)
    ...
    kcc_share_other_expenses2=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)
...
class QuarterlyBills(forms.Form):
    ...
    billing_date=forms.DateField(widget=forms.DateInput(attrs={'type': 'date', 'class':'form-control'}), label='Date of Billing ')
...
class AdditionalFieldsFirstMonth(forms.Form):
    ...
    change_rt_cash_desc_first_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit RT Cash (KCC) Description', required=False)
    change_rt_cash_amount_first_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit RT Cash (KCC) Amount", required=False)
    ...
    change_partial_cash_desc_first_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Partial Cash (KCC) Description', required=False)
    change_partial_cash_amount_first_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Partial Cash (KCC) Amount", required=False)
    ...
    change_mjpjay_desc_first_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit MJPJAY (KCC) Description', required=False)
    change_mjpjay_amount_first_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit MJPJAY (KCC) Amount", required=False)
    ...
    change_pmjay_desc_first_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit PMJAY (KCC) Description', required=False)
    change_pmjay_amount_first_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit PMJAY (KCC) Amount", required=False)
    ...
    change_police_file_desc_first_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Police File (KCC) Description', required=False)
    change_police_file_amount_first_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Police File (KCC) Amount", required=False)
    ...
    change_insurance_desc_first_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Insurance (KCC) Description', required=False)
    change_insurance_amount_first_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Insurance (KCC) Amount", required=False)
    ...
    change_other_income_desc_first_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Other Income (KCC) Description', required=False)
    change_other_income_amount_first_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Other Income (KCC) Amount", required=False)
    ...
    change_ct_kcc_desc_first_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit CT (KCC) Description', required=False)
    change_ct_kcc_amount_first_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit CT (KCC) Amount", required=False)
    ...
    change_mould_kcc_desc_first_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Mould (KCC) Description', required=False)
    change_mould_kcc_amount_first_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Mould (KCC) Amount", required=False)
    change_additional_income_desc_first_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Additional Income (KCC) Description', required=False)
    kcc_share_add_in1_first_month=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)
    ...
    kcc_share_add_in2_first_month=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)
    change_other_expenses_desc_first_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Other Expenses (KCC) Description', required=False)
    kcc_share_other_expenses1_first_month=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)
    ...
    kcc_share_other_expenses2_first_month=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)
...
class AdditionalFieldsSecondMonth(forms.Form):
    ...
    change_rt_cash_desc_second_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit RT Cash (KCC) Description', required=False)
    change_rt_cash_amount_second_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit RT Cash (KCC) Amount", required=False)
    ...
    change_partial_cash_desc_second_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Partial Cash (KCC) Description', required=False)
    change_partial_cash_amount_second_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Partial Cash (KCC) Amount", required=False)
    ...
    change_mjpjay_desc_second_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit MJPJAY (KCC) Description', required=False)
    change_mjpjay_amount_second_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit MJPJAY (KCC) Amount", required=False)
    ...
    change_pmjay_desc_second_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit PMJAY (KCC) Description', required=False)
    change_pmjay_amount_second_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit PMJAY (KCC) Amount", required=False)
    ...
    change_police_file_desc_second_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Police File (KCC) Description', required=False)
    change_police_file_amount_second_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Police File (KCC) Amount", required=False)
    ...
    change_insurance_desc_second_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Insurance (KCC) Description', required=False)
    change_insurance_amount_second_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Insurance (KCC) Amount", required=False)
    ...
    change_other_income_desc_second_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Other Income (KCC) Description', required=False)
    change_other_income_amount_second_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Other Income (KCC) Amount", required=False)
    ...
    change_ct_kcc_desc_second_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit CT (KCC) Description', required=False)
    change_ct_kcc_amount_second_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit CT (KCC) Amount", required=False)
    ...
    change_mould_kcc_desc_second_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Mould (KCC) Description', required=False)
    change_mould_kcc_amount_second_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Mould (KCC) Amount", required=False)
    change_additional_income_desc_second_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Additional Income (KCC) Description', required=False)
    kcc_share_add_in1_second_month=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)
    ...
    kcc_share_add_in2_second_month=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)
    change_other_expenses_desc_second_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Other Expenses (KCC) Description', required=False)
    kcc_share_other_expenses1_second_month=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)
    ...
    kcc_share_other_expenses2_second_month=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)

class AdditionalFieldsThirdMonth(forms.Form):
    change_rt_cash_desc_third_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit RT Cash (KCC) Description', required=False)
    change_rt_cash_amount_third_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit RT Cash (KCC) Amount", required=False)
    ...
    change_partial_cash_desc_third_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Partial Cash (KCC) Description', required=False)
    change_partial_cash_amount_third_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Partial Cash (KCC) Amount", required=False)
    ...
    change_mjpjay_desc_third_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit MJPJAY (KCC) Description', required=False)
    change_mjpjay_amount_third_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit MJPJAY (KCC) Amount", required=False)
    ...
    change_pmjay_desc_third_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit PMJAY (KCC) Description', required=False)
    change_pmjay_amount_third_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit PMJAY (KCC) Amount", required=False)
    ...
    change_police_file_desc_third_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Police File (KCC) Description', required=False)
    change_police_file_amount_third_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Police File (KCC) Amount", required=False)
    ...
    change_insurance_desc_third_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Insurance (KCC) Description', required=False)
    change_insurance_amount_third_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Insurance (KCC) Amount", required=False)
    ...
    change_other_income_desc_third_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Other Income (KCC) Description', required=False)
    change_other_income_amount_third_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Other Income (KCC) Amount", required=False)
    ...
    change_ct_kcc_desc_third_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit CT (KCC) Description', required=False)
    change_ct_kcc_amount_third_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit CT (KCC) Amount", required=False)
    ...
    change_mould_kcc_desc_third_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Mould (KCC) Description', required=False)
    change_mould_kcc_amount_third_month=forms.DecimalField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="Edit Mould (KCC) Amount", required=False)
    change_additional_income_desc_third_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Additional Income (KCC) Description', required=False)
    kcc_share_add_in1_third_month=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)
    ...
    kcc_share_add_in2_third_month=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)
    change_other_expenses_desc_third_month=forms.CharField(widget=forms.TextInput(attrs={'class':'form-control'}), label='Edit Other Expenses (KCC) Description', required=False)
    kcc_share_other_expenses1_third_month=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)
    ...
    kcc_share_other_expenses2_third_month=forms.IntegerField(widget=forms.NumberInput(attrs={'class':'form-control'}), label="KCC's Share (%)", required=False)